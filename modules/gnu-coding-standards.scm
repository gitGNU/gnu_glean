...
(define-module (modules gnu-coding-standards)
...
(define gcs-gmodule
  (gmodule
   (id 'gnu-coding-standards)
   (parts `(,prefix ,exec_prefix ,bindir ,libdir ,includedir
		    ,datarootdir ,datadir ,mandir ,infodir ,docdir))
   (info
    (name "The GNU Coding Standards")
    (version "0.1")
    (synopsis "Fundamentals of the GNU Coding Standards.")
...
(define prefix
  (gset 'prefix
	(problem
	 (q "Name the variable defining the overall prefix for the
installation")
	 (s "prefix"))
	(problem
	 (q "The variable defining the first part of the
installation's target path is:")
	 (s "prefix")
	 (s "prefix")
	 (s "exec_prefix")
	 (s "target"))
	(problem
	 (q "Given the path '/usr/local/man', what is the prefix part
used by default?")
	 (s "/usr/local")
	 (s "/usr")
	 (s "/usr/local")
...
present")))))

(define gcs-description
  "It is a truth universally acknowledged, that as a
