...
# # GUILECRAFT_CHECK_ARTANIS
# # dnl
# # dnl Check whether Nala Ginrut's Artanis has already been installed.
AC_DEFUN([GUILECRAFT_CHECK_ARTANIS], [
  AC_CACHE_CHECK([whether (artanis artanis) is already installed],
    [ac_cv_guilecraft_artanis_installed],
    [if "$GUILE" -c "(use-modules (artanis artanis))"
     then
       ac_cv_guilecraft_artanis_installed=yes
     else
       ac_cv_guilecraft_artanis_installed=no
     fi])
])
